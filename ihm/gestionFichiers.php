...
$repertoire = "../donnees/fichiers";
    if ($dossier = opendir ($repertoire)) {
        while ( false !== ($file = readdir ( $dossier )) ) {
            ...
            if ($fileExtension != "" && $file != '_fichier_defaut_[0000].pdf') {
                $affichageListeFichiers .= '<span class=texte><a target="_blank" href="'.$repertoire.'/'.utf8_encode ($file).'">'.utf8_encode ($file).'</a><br>(cliquez pour afficher ou télécharger le fichier)</span>';
                if (!array_key_exists(utf8_encode ($file), $listeFichiersStockes)){
                    $affichageListeFichiers .= '<span class=texte>Fichier non utilisé</span>';
                    $affichageListeFichiers .= '<br>';
                    $affichageListeFichiers .= '<a class=bouton_repertoire name=supprimer_fichier data-nomFichier="'.utf8_encode ($file).'" data-nomRepertoire="'.$repertoire.'" href="">Supprimer ce fichier du répertoire</a>';
                }
                else {
